...
def send_message(bot, chat_id, text):
  try:
    ...
    log_text('No such chat_id available to bot')
...
def check_new_webworks(bot, chat_id):
  chat_info = api_calls.get_chat_info()
...
def set_webworks(webworks):
  api_calls.update_webworks(webworks)
...
def notify_webwork(bot, update):
  ...
  if not ('username' in chat_info and 'webwork_password' in chat_info):
...
def notify_grades(bot, update):
  # TODO: Refactor long if statement

  chat_id = update.message.chat_id
  send_chatting_action(bot, chat_id)
  chat_info = api_calls.get_chat_info()

  if not ('username' in chat_info and 'main_password' in chat_info):
    ...
    if len(current_grades.keys()) == 0:
      ...
      set_grades(current_grades)


def set_grades(new_grades):
  api_calls.update_grades(new_grades)
  ...
  if not ('username' in chat_info and 'main_password' in chat_info):
    ...
    try:
      ...
      if len(schedule.keys()) == 0:
        ...
        api_calls.update_schedule(schedule)
    except:
...
def next_lecture(bot, update):
  # TODO: Think if you can refactor the code
...
def notify_minutes_choice(bot, update):
  ...
  try:
    ...
    api_calls.update_schedule_notify_minutes(minutes)
...
def notifying_webworks_process(bot):
  while True:
    if time_helpers.current_time_in_minutes() >= 0 and time_helpers.current_time_in_minutes() < 720:
      continue
...
def get_right_word(notify_minutes):
  right_word = 'минут'

  if (notify_minutes % 10 == 1):
    right_word = 'минуту'

  if (notify_minutes % 10 > 1 and notify_minutes % 10 < 5):
    right_word = 'минуты'
...
def notifying_lectures_process(bot):
  # TODO: Rewrite it with scheduling
  # TODO: See if you can refactor the code
  ...
  while True:
    ...
    for subject in schedule[current_day]:
      ...
      if subject_start_time - current_minutes == notify_minutes:
        right_word = get_right_word(notify_minutes)
...
def notifying_grades_process(bot):
  ...
  while True:
    ...
    try:
      ...
      log_text('Checking {} grades..'.format(username))
      ...
      if len(current_grades.keys()) == 0:
        ...
        api_calls.disable_notify_grades(chat_id)
        return
...
def feedback_choice(bot, update):
  feedback_username = update.message.chat.username
  feedback_text     = update.message.text

  final_feedback = '@{} has left feedback from local version of indigo 📝\n\n{}'.format(feedback_username, feedback_text)
